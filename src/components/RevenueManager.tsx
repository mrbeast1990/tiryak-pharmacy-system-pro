    ...
    try {
      ...
      if (dayRevenues.length === 0) {
        ...
        dayRevenues.forEach(rev => {
          ...
          doc.text(`${rev.income.toFixed(2)} LYD`, 80, yPosition);
          doc.text(`${rev.expense.toFixed(2)} LYD`, 120, yPosition);
          doc.text(`${(rev.income - rev.expense).toFixed(2)} LYD`, 160, yPosition);
          
          totalRevenue += rev.income;
          totalExpense += rev.expense;
          yPosition += 12;
        });
        ...
        yPosition += 10;
        doc.setFontSize(12);
        doc.text(`Total Revenue: ${totalRevenue.toFixed(2)} LYD`, 30, yPosition);
        yPosition += 12;
        doc.text(`Total Expense: ${totalExpense.toFixed(2)} LYD`, 30, yPosition);
        yPosition += 12;
        doc.text(`Net Profit: ${(totalRevenue - totalExpense).toFixed(2)} LYD`, 30, yPosition);
      }
      
      doc.save(`daily-revenue-${date}.pdf`);
      ...
      if (periodRevenues.length === 0) {
        ...
        Object.keys(groupedByDate).sort().forEach(date => {
          ...
          groupedByDate[date].forEach(rev => {
            ...
            dailyRevenue += rev.income;
            dailyExpense += rev.expense;
            yPosition += 12;
          });
          
          // Daily total
          doc.setFontSize(12);
          doc.text(`Daily Total: ${(dailyRevenue - dailyExpense).toFixed(2)} LYD`, 30, yPosition);
        ...
        // Grand total
        doc.setFontSize(14);
        doc.text(`Total Revenue: ${totalRevenue.toFixed(2)} LYD`, 20, yPosition);
        yPosition += 12;
        doc.text(`Total Expense: ${totalExpense.toFixed(2)} LYD`, 20, yPosition);
        yPosition += 12;
        doc.text(`Grand Total: ${(totalRevenue - totalExpense).toFixed(2)} LYD`, 20, yPosition);
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center">
              ...
              <h1 className="text-lg font-bold text-gray-900 mr-4">{t('revenue.title')}</h1>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <Card className="card-shadow mt-6">
              <CardHeader>
                <CardTitle className="text-lg">{language === 'ar' ? `إجمالي يوم ${date}` : `Daily Total ${date}`}</CardTitle>
              </CardHeader>
