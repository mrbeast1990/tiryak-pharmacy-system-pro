-- تعديل وظيفة إشعار النقص لإظهار اسم الصنف فقط للمستخدمين العاديين وأحمد الرجيلي
...
BEGIN
  ...
  IF NEW.status = 'shortage' THEN
    ...
    -- Send notification to admins only with detailed message
    ...
    -- Send to ahmad_rajili with basic message (like regular users)
    PERFORM public.send_notification_to_role(
      'نقص جديد في المخزون',
      user_message,
      'ahmad_rajili',
